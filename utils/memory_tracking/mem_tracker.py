"""
reference https://github.com/Oldpan/Pytorch-Memory-Utils
"""
import gc
import datetime
import pynvml
...
class MemTracker(object):
    ...
    """
    def __init__(self, frame, detail=True, verbose=False, device=0):
        self.frame = frame
    ...
    def track(self):
        ...
        if self.begin:
            logging.info(f"\nGPU Memory Track | {datetime.datetime.now():%d-%b-%y-%H:%M:%S} |"
        ...
        if self.print_detail is True:
            ...
            for t, s, n, m in new_tensor_sizes - self.last_tensor_sizes:
                logging.info(f'+ | {str(n)} * Size:{str(s):<20} | Memory: {str(m*n)[:6]} M | {str(t):<20}\n')
            for t, s, n, m in self.last_tensor_sizes - new_tensor_sizes:
                logging.info(f'- | {str(n)} * Size:{str(s):<20} | Memory: {str(m*n)[:6]} M | {str(t):<20} \n')
            self.last_tensor_sizes = new_tensor_sizes

        logging.info(f"\nThe above is at {where_str:<50} - "
            f"Total Used Memory:{meminfo.used/1000**2:<7.1f}Mb\n\n")
